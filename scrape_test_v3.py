#!/usr/bin/env python3
"""GitHub Actions Playwright 抓取脚本 V3（expect_response 优化）

改进：使用 page.wait_for_response() 精确等待 API 响应，替代固定等待时间。
"""
import argparse
import datetime
import json
...
def scrape_one(page, goods_id, item_name=None):
    ...
    try:
        # 1. 访问详情页 + 等待 info/good 响应
        print(f"  [1] 访问详情页（等待 API）...", flush=True)
        try:
            with page.expect_response(
                lambda r: "info/good" in r.url and r.status == 200,
                timeout=30000
            ) as resp_info:
            ...
            print(f"      info/good 等待失败: {e}", flush=True)
            page.wait_for_timeout(3000)

        # 2. 点击 K 线图 + 等待 chartAll 响应
        print(f"  [2] 点击 K 线图（等待 API）...", flush=True)
        try:
            with page.expect_response(
                lambda r: "chartAll" in r.url and r.status == 200,
                timeout=15000
            ):
                page.evaluate("""() => {
        ...
        # 3. 切换平台到悠悠有品 + 等待 chartAll 响应
        print(f"  [3] 切换平台到悠悠有品（等待 API）...", flush=True)
        ...
        # 4. 切换日线 + 等待 chartAll 响应
        print(f"  [4] 切换日线（等待 API）...", flush=True)
        try:
            with page.expect_response(
                lambda r: "chartAll" in r.url and r.status == 200,
                timeout=15000
        ...
        # 5. 翻页（每次等待 chartAll 响应）
        print(f"  [5] 翻页（等待 API）...", flush=True)
        ...
        # 6. 切换 1 小时 + 等待 chartAll 响应
        print(f"  [6] 切换 1 小时（等待 API）...", flush=True)
        try:
            with page.expect_response(
                lambda r: "chartAll" in r.url and r.status == 200,
                timeout=15000
        ...
        # 7. 筹码分布 + 等待 chipData 响应
        print(f"  [7] 点击筹码分布图（等待 API）...", flush=True)
        try:
            with page.expect_response(
                lambda r: "chipData" in r.url and r.status == 200,
                timeout=15000
            ) as resp:
                page.evaluate("""() => {
...
def main():
    parser = argparse.ArgumentParser(description="CSQAQ Playwright 抓取 V3")
    parser.add_argument("--items-json", default="", help="批量 JSON 数组")
    args = parser.parse_args()

    print("=" * 60, flush=True)
    print("  CSQAQ Playwright 抓取 V3（expect_response 优化）", flush=True)
    print("=" * 60, flush=True)
