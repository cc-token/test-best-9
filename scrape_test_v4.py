#!/usr/bin/env python3
"""GitHub Actions Playwright 抓取脚本 V4（networkidle 优化）

改进：使用 wait_for_load_state("networkidle") 等待网络空闲，替代固定等待时间。
页面加载完成后立即继续，不浪费时间。
"""
import argparse
import datetime
...
def main():
    parser = argparse.ArgumentParser(description="CSQAQ Playwright 抓取 V4")
    parser.add_argument("--items-json", default="", help="批量 JSON 数组")
    args = parser.parse_args()

    print("=" * 60, flush=True)
    print("  CSQAQ Playwright 抓取 V4（networkidle 优化）", flush=True)
    print("=" * 60, flush=True)
